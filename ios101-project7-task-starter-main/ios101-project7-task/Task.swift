...
extension Task {
    ...
    static func getTasks() -> [Task] {
        ...
        if let data = defaults.data(forKey: "tasks")
        {
            ...
            // 4.
            return decodedData // returns decoded tasks
        } else
        {
            // 5.
            return []
    ...
    func save() 
{
    ...
    // Check if the current task already exists in the array
        if let index = tasks.firstIndex(where: { $0.id == self.id }) {
            // If it exists, update the existing task
            tasks[index] = self
        } else {
        ...
        // Save the modified array of tasks to UserDefaults.
                Task.save(tasks)
